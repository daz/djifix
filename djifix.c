  ...
  */
  {
	...
	*/
	...
	fprintf(stderr, "\n(Skipping over anomalous bytes (nalSize 0x%X), starting at file position 0x%lx)...\n", nalSize, ftell(inputFID) - 4);
	do {
	  if (!get1Byte(inputFID, &c)) return;
	  nalSize = (nalSize<<8)|c;
	} while (nalSize != 2);
	fprintf(stderr, "...resuming at file position 0x%lx).  Continuing to repair the file (please wait)...", ftell(inputFID) - 4);
      }
    }
  }
}
  ...
  */
  {
    ...
    */
    while (1) {
      ...
      fprintf(stderr, "\tIf the video format was H.265, 2160(x3840)p(UHD-1), 25fps: Type 8, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.264, 1530p, 30fps: Type 9, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.264, 1530p, 24fps: Type a, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.265, 1080p, 120fps: Type b, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.264, 1080p, 120fps: Type c, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.264, 1080p, 30fps: Type d, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.265, 1080p, 25fps: Type e, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.264, 1080p, 25fps: Type f, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.264, 1080p, 24fps: Type g, then the \"Return\" key.\n");
      fprintf(stderr, "\tIf the video format was H.264, 480p, 30fps (e.g., from a XL FLIR camera): Type h, then the \"Return\" key.\n");
      fprintf(stderr, " If the resulting file is unplayable by VLC, then you may have guessed the wrong format;\n");
      fprintf(stderr, " try again with another format.)\n");
      fprintf(stderr, "If you know for sure that your video format was *not* one of the ones listed above, then please email \"djifix@live555.com\", and we'll try to update the software to support your video format.\n");
      do {formatCode = getchar(); } while (formatCode == '\r' && formatCode == '\n');
      if ((formatCode >= '0' && formatCode <= '9') ||
	  (formatCode >= 'a' && formatCode <= 'h') ||
	  (formatCode >= 'A' && formatCode <= 'H')) {
	break;
      }
      fprintf(stderr, "Invalid entry!\n");
    ...
    switch (formatCode) {
      ...
      case '8': { sps = type3_H265_SPS_2160x3840p25; pps = type3_H265_PPS_2160x3840p25; vps = type3_H265_VPS_2160x3840p25; break; }
      case '9': { sps = type3_H264_SPS_1530p30; pps = type3_H264_PPS_default; break; }
      case 'a': case 'A': { sps = type3_H264_SPS_1530p24; pps = type3_H264_PPS_default; break; }
      case 'b': case 'B': { sps = type3_H265_SPS_1080p120; pps = type3_H265_PPS_1080p120; vps = type3_H265_VPS_1080p; break; }
      case 'c': case 'C': { sps = type3_H264_SPS_1080p120; pps = type3_H264_PPS_default; break; }
      case 'd': case 'D': { sps = type3_H264_SPS_1080p30; pps = type3_H264_PPS_default; break; }
      case 'e': case 'E': { sps = type3_H265_SPS_1080p25; pps = type3_H265_PPS_1080p25; vps = type3_H265_VPS_1080p; break; }
      case 'f': case 'F': { sps = type3_H264_SPS_1080p25; pps = type3_H264_PPS_default; break; }
      case 'g': case 'G': { sps = type3_H264_SPS_1080p24; pps = type3_H264_PPS_default; break; }
      case 'h': case 'H': { sps = type3_H264_SPS_480p30; pps = type3_H264_PPS_480p; break; }
